...
class Contactforms extends CI_Controller {
	...
	function __construct() {
        ...
        if(getSessionData() =='true'){
			$this->load->model('contactforms/App_users_model');
			return true;
		}else{
			redirect('admin');
		}
	...
	public function index(){
		...
		$data['main_content'] = 'contactforms/main_users';
		$this->load->view('admin/layout', $data);
	}
	
	public function contact(){
		$data['resultset'] = $this->App_users_model->getUserlistC();
		$data['main_content'] = 'contactforms/index';
		$this->load->view('admin/layout', $data);
	}
	...
	public function edit($id){
		if($id){
			$data['admin_user'] = $this->App_users_model->EditTestimonials($id);
			$data['main_content'] = 'contactforms/add_users';
			$this->load->view('admin/layout', $data);
		}
	...
	public function cedit($id){
		if($id){
			$data['admin_user'] = $this->App_users_model->EditContacs($id);
			$data['main_content'] = 'contactforms/editcontact';
			$this->load->view('admin/layout', $data);
		}
		
	}
 
	public function deletetemessage($id=false){
		if($id){
			$result = $this->App_users_model->deletetemessage($id);
		if($result){
				$this->session->set_flashdata('success_message', lang('delete_message'));
				redirect('contactforms');
			}else{
	...
	public function deletetecontact($id=false){
		if($id){
			$result = $this->App_users_model->deleteUserc($id);
			if($result){
				$this->session->set_flashdata('success_message', lang('delete_message'));
				redirect('contactforms/contact');
			}else{
				$this->session->set_flashdata('error_message', lang('error_message'));
				redirect('contactforms/contact');
			}
