...
class Regionmodel extends CI_Model{
	...
	protected $tbl_Module = 'tbl_nationality';
	...
	public function updatelocation_page($sub_ad, $id) {
		...
		$update_status = $this->db->update('tbl_nationality', $sub_admin);
		if ($update_status) {
	...
	public function getCountryDetail1($array) {
		//pr($array);DIE;
		$this->db->select('tbl_font_country.font_id,tbl_font_country.country_id,tbl_nationality.country_id,tbl_nationality.country_name');
    $this->db->from('tbl_font_country');
    $this->db->join('tbl_nationality', 'tbl_nationality.country_id = tbl_font_country.country_id');
